    ...
    public class GraphLabel
    {
        ...
        public bool isOver(Graphics g, Point pt)
        {
            try
            {
                ...
                g.TranslateTransform((float)Common.FrmServer.pnlMain.Width / (float)2, (float)Common.FrmServer.pnlMain.Height / (float)2);
                g.TranslateTransform(location.X, location.Y);
        ...
        public void setupBoundingBox(RectangleF[] r, Graphics g, string label, float value, float degree)
        {
            try
            {
                ...
                if (r != null)
                {
                    ...
                    while (counter > -Common.FrmServer.circleDiameter && go)
                    {
                        go = false;
        ...
        public void setupBoundingBox2(int counter, Graphics g, float value)
        {
            try
            {
                float tempDistance2 = Common.FrmServer.circleRadius + counter + Common.dataUnit * ((float)value - (float)Common.minControlPointValue);

                location = new PointF(tempDistance2 * (float)Math.Cos(degree * (Math.PI / 180)),
                                           tempDistance2 * (float)Math.Sin(degree * (Math.PI / 180)) - 7);

                SizeF sf = g.MeasureString(label, Common.FrmServer.f10, Common.FrmServer.pnlMain.Width, Common.FrmServer.fmt);
                sf.Width += 2;
                sf.Height += 2;
